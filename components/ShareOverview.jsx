  ...
  return (
    <View className={`rounded-lg p-4 m-4 bg-background-${colorScheme}`}>
      ...
      <Text className={`text-lg text-text-${colorScheme} mb-2`}>
        Total Shares Owned:{" "}
        {numberWithCommas(
          shares.reduce((total, current) => total + current.quantity, 0)
        )}
      </Text>
      <Text className={`text-lg text-text-${colorScheme} mb-4`}>
        ...
        {numberWithCommas(
          shares.reduce(
            (total, current) => total + current.quantity * current.games.value,
            0
          )
        )}
      </Text>
      <View className="sm:flex-col sm:flex-row justify-between">
      ...
      <View className={`border border-text-dark rounded-lg`}>
        ...
        >
          <Text
            className={`font-bold text-text-${colorScheme} text-right flex-1`}
          >
            Game
          </Text>
          <Text
        ...
        {shares.map((share, index) => (
          <View
            ...
            key={index}
          >
            <Text className={`text-text-${colorScheme} text-right flex-1`}>
              {share.games.game_name}
            </Text>
            <Text className={`text-text-${colorScheme} text-right flex-1`}>
              {numberWithCommas(share.quantity)}
            </Text>
            <Text className={`text-text-${colorScheme} text-right flex-1`}>
              {numberWithCommas(share.games.value * share.quantity)}
            </Text>
          </View>
        ))}
      </View>
